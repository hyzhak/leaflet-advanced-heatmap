...
export default class AdvancedHeatmapLayer extends Layer {
  ...
  redraw () {
    if (this._heat && !this._frame && this._map && !this._map._animating && !this._isMoving) {
  ...
  onAdd (map) {
    ...
    map.on('move', this._handleMoveStart, this)
  ...
  onRemove (map) {
    ...
    map.off('move', this._handleMoveStart, this)
  ...
  _initCanvas () {
    ...
    this.options = merge({}, defaultHeatmapOptions, this.options.heatmap)
  ...
  _redraw () {
    ...
    // TODO: could we be more efficient and update changes only?
    this._heat.removeAll()

    for (let i = 0, len = this._latlngs && this._latlngs.length; i < len; i++) {
      ...
      if (bounds.contains(p)) {
        // scale based on lng
        const radius = scaleBy(
          latLng,
          // TODO: for the moment size is 3rd field and 4th is opacity
          // but it could be different
          pointSource[2] || this.options.radius
        )
        this._heat.add([
          Math.round(p.x),
          Math.round(p.y),
          radius,
          pointSource[3] || this.options.opacity
        ])
      }
    }
  ...
  _scaleByMeters (latlng, size = 1) {
    ...
    // to change scale to miles (for example), you will need to convert 40075017 (equatorial circumference of the Earth
    // in metres) to miles
    const map = this._map
